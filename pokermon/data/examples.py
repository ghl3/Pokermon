...
from pokermon.data.reenforcement_types import Row, Context
...
def seq_example_to_dict(example: tf.train.SequenceExample) -> Dict[str, Any]:
    feature_map = {'context': _features_to_dict(example.context),
...
def make_example(context: Context, rows: List[Row]) -> tf.train.Example:
    ...
    for row in rows:
        ...
        feature_map.update(with_prefix('state', _make_feature_map(Row.State, row.state)))
        feature_map.update(with_prefix('action', _make_feature_map(Row.Action, row.action)))
        feature_map.update(with_prefix('reward', _make_feature_map(Row.Reward, row.reward)))
