from pokermon.poker.cards import mkhand, mkboard
...
def test_evaluation_vs_hand_with_board():
    res = odds_vs_random_hand(hand=mkhand("AcAd"), other_hand=mkhand('KcKs'),
                              board=mkboard("3s5s7s"))
    assert 0.52 < res.win_rate() < 0.62

    res = odds_vs_random_hand(hand=mkhand("5s8s"), other_hand=mkhand('Tc9d'),
                              board=mkboard("6h7cTd"))
